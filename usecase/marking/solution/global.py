    ...
    for direction in particle.direction:
        ...
        if location_exists(graph, adjacent_location_coords):
            if get_location_with_coords(graph, adjacent_location_coords) in particle.current_location.adjacent.values():
                continue
            particle.current_location.adjacent[temp_dir] = get_location_with_coords(graph, adjacent_location_coords)
            continue
        ...
        add_location_to_graph(sim, graph, new_location, particle.direction)
...
def solution(sim):
    ...
    sim.csv_round_writer.set_communication_range("Nan")
    ...
    for particle in sim.get_particle_list():
        ...
        if sim.get_actual_round() == 1:
            ...
            add_location_to_graph(sim, graph, particle.current_location, particle.direction)
            discover_adjacent_locations(sim, particle)
            continue
